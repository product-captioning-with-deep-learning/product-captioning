...
'''
#### NOTICE ####
The original code : https://github.com/suragnair/seqGAN
We has followed this code. What we fix on this code is just some hyperparameter(just below this paragraph), '<start>', '<end>', '<unk>' token index.
...
pretrained_gen_path = "seq30_b64_dim_200_v12028_mlep400, advp20_posnef_10700_gen.pth"
...
print("save oracle")
torch.save(oracle.state_dict(), pretrained_oracle_path)
...
print("before")
train_generator_MLE(gen, gen_optimizer, oracle, oracle_samples, 10)
print("after")
...
print("save gen")
torch.save(gen.state_dict(), pretrained_gen_path[:-4] + "_adv.pth")
# gen.load_state_dict(torch.load(pretrained_gen_path))
print("save dis")
torch.save(dis.state_dict(), pretrained_dis_path[:-4] + "_adv.pth")
# dis.load_state_dict(torch.load(pretrained_dis_path))

for i in gen.sample(10):
